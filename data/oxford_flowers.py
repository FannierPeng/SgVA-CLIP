...
class OxfordFlowers(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False, subsample='all', num_shots=1):
        ...
        self.phase = phase
        self.name = 'DTD_' + phase
        self.image_dir = os.path.join(_IMAGE_DATASET_DIR, "jpg")
        self.split_fewshot_dir = os.path.join(_IMAGE_DATASET_DIR, "split_fewshot")
        self.label_file = os.path.join(_IMAGE_DATASET_DIR, "imagelabels.mat")
        self.lab2cname_file = os.path.join(_IMAGE_DATASET_DIR, "cat_to_name.json")
        self.split_path = os.path.join(_IMAGE_DATASET_DIR, "split_zhou_OxfordFlowers.json")
        seed = 42
        random.seed(seed)
        np.random.seed(seed)
