...
def ViTeCLSHead(k_all, query, new_query, support, new_support, textemb_query, textemb_support, support_labels, logit_scale, v_proj, v_bias, n_way, n_shot, is_scale=False, normalize=True):
    ...
    # print('support.shape', support.shape) #[episodes_perbatch, n*k, emb_m]
    # print('textemb_support.shape', textemb_support.shape) #[episodes_perbatch, n*k, emb_m]

    # print('textemb_support = ', textemb_support)
    text_prototypes = torch.bmm(labels_train_transposed, textemb_support) #矩阵乘，得到[episodes_perbatch , n , emb_m]
    # Divide with the number of examples per novel category.
    text_prototypes = text_prototypes.div(
        labels_train_transposed.sum(dim=2, keepdim=True).expand_as(text_prototypes)
    )
    ...
    logits_text2img = logit_scale * torch.bmm(textemb_query, image_prototypes.transpose(1, 2))
    logits_text2text = logit_scale * torch.bmm(textemb_query, text_prototypes.transpose(1, 2))
