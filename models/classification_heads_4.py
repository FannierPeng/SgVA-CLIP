...
def ViTeCLSHead(k_all, query, new_query, support, new_support, image_prototypes, new_image_prototypes, textemb_support, textemb_query, support_labels, logit_scale, v_proj, v_bias, n_way, n_shot, n_classes, query_unique_id, labels_query_unique, is_scale=False, normalize=True):
    """
    Constructs the prototype representation of each class(=mean of support vectors of each class) and
    returns the classification score (=L2 distance to each class prototype) on the query set.
...
#         print('textemb_query',textemb_query.shape)
        short_text_prototypes = None
        if labels_query_unique !=None:
#             print('labels_query_unique',labels_query_unique)
#             print('support_labels',support_labels)
            labels_query_unique=labels_query_unique.reshape(-1)
            textemb_query = text_prototypes.index_select(1, labels_query_unique).view(tasks_per_batch, n_shot*n_way, -1)

            short_text_prototypes = torch.bmm(labels_train_transposed, textemb_query)
            short_text_prototypes = short_text_prototypes.div(
                labels_train_transposed.sum(dim=2, keepdim=True).expand_as(short_text_prototypes)).half()
...
#         print('query_unique_id',query_unique_id)
        ...
        logits_img2text_unique = logit_scale * query_unique@(text_prototypes.expand(n_shot, n_classes, query_unique.shape[-1]).transpose(1, 2))
    else:
        logits_img2text_unique = None
    
    logits_img2text = logit_scale * query@text_prototypes.transpose(1, 2)
    if labels_query_unique !=None:
        logits_img2text_short = logit_scale * query@short_text_prototypes.transpose(1, 2)
    ...
    if new_query != None:
        ...
        if new_query.shape[-1] != text_prototypes.shape[-1]:
            if v_bias == None:
                logits_new_img2text = logit_scale * (new_query@v_proj)@text_prototypes.transpose(1, 2)
                logits_old_img2img = None
            else:
                logits_new_img2text = logit_scale * (new_query@v_proj.t()+v_bias)@text_prototypes.transpose(1, 2)
                logits_old_img2img = None
        else:
            logits_new_img2text = logit_scale * new_query@text_prototypes.transpose(1, 2)
    ...
    return logits_img2text, logits_text2img, logits_img2img, logits_img2fuse, logits_text2text, logits_new_img2img, logits_new_img2text, logits_old_img2img, logits_img2text_unique, logits_img2text_short
...
class ClassificationHead(nn.Module):
    ...
    def forward(self, k_all, query, new_query, support, new_support, image_prototypes, new_image_prototypes, textemb_support, textemb_query, support_labels, logit_scale, v_proj, v_bias, n_way, n_shot, n_classes, query_unique_id, labels_query_unique,*kwargs):
        if self.enable_scale:
            return self.scale * self.head(k_all, query, new_query, support, new_support, image_prototypes, new_image_prototypes, textemb_support, textemb_query, support_labels, logit_scale,  v_proj, v_bias, n_way, n_shot, n_classes, query_unique_id, labels_query_unique,*kwargs)
        else:
            return self.head(k_all, query, new_query, support, new_support, image_prototypes, new_image_prototypes, textemb_support, textemb_query, support_labels, logit_scale, v_proj, v_bias, n_way, n_shot, n_classes, query_unique_id, labels_query_unique,*kwargs)