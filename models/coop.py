...
class Weight_Adapter(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.alpha = nn.Parameter(torch.FloatTensor([0.5])).to(dtype)
...
class PromptLearner(nn.Module):
    ...
    def forward(self, class_ids):
        ...
        tokenized_prompts = None #self.tokenized_prompts
        whole_prompts = None
        if ctx.dim() == 2:
        ...
        if self.para_update == 'v_adapter' or self.para_update == 'prompt+v_adapter':
            if 'T' in self.visual_pre:
                if 'ViT' in self.backbone:
                    new_image_features = self.adapter(image_features@torch.pinverse(self.image_encoder.proj))
                else:
                    with torch.no_grad():
                        new_image_features = (image_features-self.image_encoder.attnpool.c_proj.bias)@torch.pinverse(self.image_encoder.attnpool.c_proj.weight.t())
                    new_image_features = self.adapter(new_image_features)
#                 if self.proj == 'projup':
#                     new_image_features_proj = new_image_features @ self.map
            ...
            if 'T' in self.visual_pre:
                if 'ViT' in self.backbone:
                    new_image_features = image_features@torch.pinverse(self.image_encoder.proj)
                else:
                    with torch.no_grad():
                        new_image_features = (image_features-self.image_encoder.attnpool.c_proj.bias)@torch.pinverse(self.image_encoder.attnpool.c_proj.weight.t())
                new_image_features = new_image_features / new_image_features.norm(dim=-1, keepdim=True)
